...
# crash_grouping2 <- read_rds("philadelphia-crash-map/shiny/app-data/crash_group2.rds")
# philly <- read_rds("philadelphia-crash-map/shiny/app-data/philly.rds")

# crash_grouping2 <-crash_grouping2 %>% 
#   ungroup() %>% 
#   mutate(crash_month = as.integer(crash_month))

pal <- colorFactor(c("navy", "red"), domain = c("No", "Yes"))

greenLeafIcon <- makeIcon(
  iconUrl = "collide-512.png",
  ...
  output$mymap <- renderLeaflet({
    leaflet(selected_crash_grouping2()) %>% 
      ...
      addRectangles(
        ...
        fillOpacity = 0.3, popup = ~as.character(risk_level),
        layerId = ~layerId)%>% 
      addMarkers(data = philly_crash_sites(), lng = ~longitude, lat = ~latitude, icon = greenLeafIcon,
                 popup = paste("<strong>Road Condition: </strong>", 
                               philly_crash_sites()$road_condition, "<br>",
                               "<strong>Fatality?: </strong>", 
                               philly_crash_sites()$fatality, "<br>",
                               "<strong>Number of Vehicles Invovled in Accident: </strong>", 
                               philly_crash_sites()$total_unit_count, "<br>",
                               "<strong>Property Damage?: </strong>", 
                               philly_crash_sites()$property_damage_only, "<br>")) %>% 
...
## fillOpacity = 0.4,
##radius = ~ifelse(fatality == "Yes", 10, 6),
##color = ~pal(fatality),
##stroke = FALSE